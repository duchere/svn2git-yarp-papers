...
The first attempt to reach the target consists of using (\ref{Eq:reaching2})
...
kinematic function (\ref{Eq:forward}) were perfectly represented and if the target were reachable, then we would have $\mathbf x_{hand} =  \mathbf x_{target}$, which implies that the target-hand Cartesian distance 
 ...
 (\ref{Eq:forward}) cannot exactly represent the system's kinematics\footnote{Part of the representational 
 errors are related to the representation of the kinematic function, in this case the
 so called Receptive Field Weighted Regression model. Part are due to the mechanical backlash of the structure.}. Therefore, even though we can find $\q_{arm}$ such that $\mathbf x_{hand}=
