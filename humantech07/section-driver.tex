...
The latter defines only two methods to manage the initialization and 
un-initialization of the device: 
...
function as a list of key-value entries (a \emph{Searchable} object). 
...
specify the type of device they want to create and its initialization 
parameters. Care must be taken to avoid that 
this introduces device dependent code. A common software engineering 
practice is to \emph{localize object creation} so to minimize the amount 
of code that is responsible for object creation and initialization. 
In YARP part of this is realized by the \emph{DeviceDriver} interface, 
which forces all initialization procedures to be 
performed inside the \emph{open} method. Device creation is instead 
