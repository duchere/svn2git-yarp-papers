...
(\ref{sec:transporting}) and on two \emph{Network Wrapper} devices, 
one acting as a \emph{Server} and the other acting as a \emph{Client}. 
Both network devices implement the very same set 
of interfaces of the device they wrap; the only difference is that they 
...
creates an instance of the \emph{Client Wrapper}. The latter exports 
exactly the same interfaces of the 
``wrapped'' device so the process is not aware that it is not connected to 
a real device. The \emph{Client Wrapper} converts calls from 
...
%
The \emph{Server Wrapper} waits for incoming connections from the network. 
In addition it creates an instance of the wrapped 
device and forwards requests from the network to the device. If requests
involve a reply theses are sent back to the 
calling port so that they are received by the remote client. The 
\emph{Server Wrapper} gains access to the local device through its 
interfaces; as such the \emph{Server Wrapper} is a total independent 
entity that can be reused for all devices implementing the same interface(s). 
%
The responsibility of the \emph{Network wrapper} devices among 
the other things is to perform the marshalling and de-marshalling of the 
