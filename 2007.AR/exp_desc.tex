...
alternative possibility is shown in \cite{shimodaira02dynamic}. This
issue is the subject of future research.}; in order to do this, since
in general not all grasps have the same length, we stretched each
sequence to a predefined lenght. The predefined length was chosen
according to the average speed of the grasps. Figure
\ref{fig:grasp_sequence} $(b)$ shows the average grasp durations for
all subjects over each experiment (moving average over $50$ grasps);
as one would expect, in general the subjects get rapidly used to the
grasp/drop/reposition task and the grasps become faster and faster. It
must be remarked, though, that this is not the case for all subjects
...
Assume $S=\{\xx_i,y_i\}_{i=1}^l$, with $\xx_i \in \RR^m$ and
$y_i \in \RR$, is a set of samples and output values; we want to find a function $f(\xx)$
such its value best predicts the associated output $y$ of any future sample $\xx$.
In the SVM framework this is done minimizing the expression:
\begin{equation} \label{eqn:svm_primal}
  \min R \left( S \right) + C \sum_{i=1}^l L^\epsilon \left( x_i, y_i, f \right)
\end{equation}
where $L^\epsilon \left( x_i, y_i, f \right)$ is our loss function and in particular
is equal to $max(0,|y_i-f(x_i)|-\epsilon)$. Hence the $\epsilon$ parameter controls the
width of the insensitive band around the
output values, e.g. training errors within this band are not considered.
The $R \left( S \right)$ is a regularization term that assures us a good generalization
power, thus the function to minimize is a trade-off between a term that measures the errors
done on the training sample and a smoothness term that avoids to overfit;
the $C$ parameter controls this trade-off.
Moreover it turns out that it is possible to implicetely non linear transform the input
values via a \emph{kernel function}. In all regression tests, a Gaussian kernel was chosen, and the
hyperparameters ($C$ and $\sigma^2$ of the gaussian) were found by grid search. The
error on regression was obtained by $2$-fold cross-validation.

For an extensive introduction to regression
