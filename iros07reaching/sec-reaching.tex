...
and arm. Instead of representing the target position in the cartesian space 
...
The remaining of the section, is divided into two parts. First we describe in details the forward motor-motor
mapping between the arm joint space and the three 
dimensional position of the hand represented in the head reference 
frame. Then we describe a visual servoing technique based on the eye-to-hand Jacobian matrix.
...
\subsubsection{Learning the open loop reaching} 
\label{sec:learning-open-loop}
%
To learn the forward motor-motor map of Eq. (\ref{Eq:forward}) we programmed 
...
A classical closed loop implementation based on the forward model 
...
Alternatively, our idea is to estimate the eye-to-hand Jacobian matrix which relates arm velocities $\dot {\mathbf q}_{arm}$
...
\end{equation}
where $k$ is an arbitrary positive scalar.
%
