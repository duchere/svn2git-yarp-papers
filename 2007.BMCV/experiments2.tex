...
and local kernels \cite{wallraven:iccv03} (SIFT). The kernels
used are infinite-dimensional, so for both kernels we run the OISVM
using different values of $\eta$.
...
an approximate incremental extension of SVM, that at
the same time attempts to reduce the number of SVs.

We benchmarked OISVM against the standard SVM formulation (offline)
and an approximate incremental SVM extension: the fixed-partition technique
\cite{syed99incremental}. In this way we can do a straightforward comparison
...
fixed-partition method. Even more important, OISVM, for these two
...
there isn't a clear plateau in
