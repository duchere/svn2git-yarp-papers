...
\begin{align}
  \min_{\ww,b} \frac{1}{2} \|\ww- \ww'\|^2 + C \sum_{i=1}^l \xi \nonumber \\
  \mbox{subject to} \;\;
...
\begin{equation}
  \ww = \beta \ww' + \sum_{i=1}^l \alpha_i \phi(\xx_i), \; \alpha_i \in \RR~, \label{eq:sol}
\end{equation}
...
error in a closed form. We show in the following that it is possible to do the
same with the modified formulation (\ref{eq:opt_prob}).

We indicate with $V_i$ the $i$'th element of the vector
$\boldsymbol{V}$ and with $M_{ij}$ the element of the matrix
$\boldsymbol{M}$ in $i$'th row and $j$'th column. We also denote with
$\boldsymbol{Y}$ the vector of the $y_i, i=1,\cdots,l$, with
$\hat{\boldsymbol{Y}}$ the vector of the predictions of the previous model
$\ww'$ on the $l$ training samples and with $\boldsymbol{K}$ the kernel matrix,
i.e. $K_{ij}=K(\xx_i,\xx_j)$.
It is possible to show that the vector $\boldsymbol{\alpha}$,
composed by $\alpha_i, i=1,\cdots,l$, in (\ref{eq:sol}) can be calculated as
\begin{align}
\boldsymbol{\alpha}&=\boldsymbol{R} (\boldsymbol{Y}-\beta \hat{\boldsymbol{Y}}) = \boldsymbol{\alpha}' - \beta \boldsymbol{\alpha}''~,
\end{align}
\noindent where $\boldsymbol{R}=(\boldsymbol{K}+1/C)^{-1}$.
From this equation, and using the same steps in \cite{Cawley06}, we have that
\begin{align}
y_i - y^{-i} = \frac{\alpha'_i}{R_{ii}} - \beta \frac{\alpha''_i}{R_{ii}}~.
\end{align}
\noindent where $y^{-i}$ is the predition on the $i$'th sample when it is
removed from the training set. In the above formula $\beta$ is the only
parameter, while the other quantities are independent from $\beta$.
Hence it is
...
adaptation.
Note that the complexity of the algorithm is dominated by the calculation
of the matrix $\boldsymbol{R}$, that is calculated in any case for the training.
So the computational cost of the leave-one-out error is negligible compared to
the training one.
