...
\end{equation}
\noindent where $\alpha_i \in \RR$ are Lagrange multipliers; hence, the final solution is given by the sum of the pre-trained model
...
We indicate with $\boldsymbol{V}_i$ the $i$'th element of the vector
$\boldsymbol{V}$ and with $\boldsymbol{M}_{ij}$ the element of the matrix
$\boldsymbol{M}$ in $i$'th row and $j$'th column.
With easy calculations it is easy to show \ref{eq:opt_prob} can be rewritten as
\begin{align}
\boldsymbol{\alpha}&=\boldsymbol{R} (\boldsymbol{Y}-\beta \hat{\boldsymbol{Y}}) \\
&= \boldsymbol{\alpha}' - \beta \boldsymbol{\alpha}''~,
\end{align}
\noindent where $\boldsymbol{R}=(\boldsymbol{K}+1/C)^{-1}$ and $\boldsymbol{K}$
is the kernel matrix, i.e. $\boldsymbol{K}_{ij}=K(\xx_i,\xx_j)$.
From this equation, and using the same steps in \cite{Cawley06}, we have that
\begin{align}
y_i - y^{-i} = \frac{\boldsymbol{\alpha}'_i}{\boldsymbol{R}_{ii}} - \beta \frac{\boldsymbol{\alpha}''_i}{R_{ii}}~.
\end{align}
\noindent where $y^{-i}$ is the predition on the $i$'th sample when it is
removed from the training set. Note that in the above formula $\beta$ is the only
