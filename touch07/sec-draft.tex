...
%
We describe a few perceptual and motor competencies required for
the robot to be able to control the body in a meaningful and safe
manner: this includes a simple attention system to spot the
objects to be grasped and the ability to control the body to reach
out for them.
%
At the end of the section we describe how these capabilities are
integrated in the grasping behavior.
...
we use an approximated inverse kinematics.

[Based in kinematics] The exploration primitives are computed
based on kinematics.
...
allows the robot to grasp objects.
