...
Spatial attention is often considered a sort of `filter'
...
Even if supported by numerous findings \cite{}, this view does not
...
This idea has been summarized by O'Regan's idea of:
...
would cheaper to memorize, for example, only a rough representation of
...
at space locations (`space-based'). Object-based attention is equivalent 
...
and very importantly of its overall goal. On the contrary, it seems 
...
The idea of object-based attention is supported by the discovery in 
monkeys of a class of neurons (\emph{mirror neurons}) which not only
...
\begin{figure}[]
	\begin{center}
		\includegraphics[width=0.77\linewidth]{./figs/attention/logpolarmapping2}
...
\section{A complete experiment}
%FRA: perche' non facciamo un esempio (senza spiegare tantissimo) dell'uso del 
%sistema per segmentare gli oggetti. Lo so che non e' tanto bello pero' se lo mettiamo
%solo come esempio si puo' giusto far vedere e spiegare la sequenza di immagini.
%Non mi piacerebbe che dicessero che l'implementazione non e' spiegata (e tanti altri
%risultati non ci sono).
%
%
\begin{center}
Visual search experiment goes here.
\end{center}
...
tensors for the orientations of $0$ and $67.5$ degrees of the central pixel,
...
discussed in details in this paper, the robot implementation
requires for example a complex management of the inhibition of return
...
The visual attention system and the object recognition
system were also used to guide the grasping action of the 
humanoid robot \cite{Orabona07}.
...
complex features to be used for object recognition.
