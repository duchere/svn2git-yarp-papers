...
The optimization of (\ref{Eq:reaching2}) can be performed numerically using various algorithms. 
...
about 2890 samples that we divided in a training set (2168 samples) and 
...
} \caption{Learning of the arm forward function. The plot reports the \emph{error}
on the test set during learning (see the text for details). The error is in degrees since $\xhand$
is a vector formed by three angles.}\label{fig:reaching-error}
\end{figure}
...
We trained the neural network on a subset of 158 elements (training set); each 
