...
From equation (\cite{eq:forward1}) it is clear that 
...
\label{eq:jacobian3}
  \mathbf{\delta \tilde{u}_hand}=
  \mathbf J\left(\mathbf q_{arm})
  \mathbf{\tilde{q}_{arm}}
\end{math}
\end{center}
%
where $\mathbf J(.)} in \mathbb \Re^{3x4}$, whose coefficients depend 
...
hand in the images ($\u_{hand} \rightarrow 0$). A possible control trategy
to achieve this convergence is to apply a joint velocity:
%
\begin{center}
\begin{math}
\dot{q}_arm=-k \cdot \mathbf J^#\u_{hand}
\end{math}
\end{center}
%
